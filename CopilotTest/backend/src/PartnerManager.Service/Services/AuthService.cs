...
namespace PartnerManager.Service.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<AuthResponse> RegisterAsync(RegisterRequest request)
        {
            ...
            return new AuthResponse { Token = token, ExpiresIn = int.Parse(_config["Jwt:ExpireMinutes"] ?? "60"), User = new UserDto { Id = user.Id, UserName = user.UserName, Email = user.Email, Role = user.Role.ToString() } };
        }

        public async Task<AuthResponse> LoginAsync(RegisterRequest request)
        {
            throw new NotImplementedException("Use LoginAsync(LoginRequest) overload in DTOs; placeholder to satisfy interface.");
        }
