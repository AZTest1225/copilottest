...
namespace PartnerManager.Api.Controllers
{
    ...
    public class PartnersController : ControllerBase
    {
        ...
        {
            var (items, total) = await _repo.GetAllAsync(search, sex, sortBy, sortOrder, page, pageSize);
            var dto = items.Select(p => new PartnerDto 
            { 
                Id = p.Id, 
                Name = p.Name, 
        ...
        public async Task<IActionResult> Create(PartnerCreateDto req)
        {
            ...
            return CreatedAtAction(nameof(Get), new { id = created.Id }, new PartnerDto 
            { 
                Id = created.Id, 
                Name = created.Name, 
